...
The `webhook.active` and `webhook.error_count` columns, their `WebhookEntity` properties and accessors, `RelatedWebhooks::updateRelated()`, `RetryWebhookMessageFailedSubscriber::failed()`, and `WebhookHealthService::recordLegacyFailure()` and `resetErrorCount()` have been removed.
