...
`WebhookEntity::$active` and `$errorCount` and their accessors, `RelatedWebhooks::updateRelated()`, `RetryWebhookMessageFailedSubscriber::failed()`, and `WebhookHealthService::recordLegacyFailure()` and `resetErrorCount()` are deprecated for removal in 6.8. Extensions should read `endpointState` from `GET /api/app-system/webhook/state` instead of the legacy fields.
